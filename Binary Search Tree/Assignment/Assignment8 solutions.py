...
class BinarySearchTree(Tree):
    ...
    def largest_to_smallest(self):
        return [x for x in self.helper(self._root)]

    def helper(self, node):
        if node._right:
            yield from self.helper(node._right)
        yield node._element
        if node._left:
            yield from self.helper(node._left)
    ...
    def iterative_search(self, node, v):
        while node is not None:
            if v == node._element:
                ...
                if node._left is None:  # Return last node searched
                    return node
                else:
                    node = node._left
            else:
                if node._right is None:  # Return last node searched
                    return node
                else:
                    node = node._right
    ...
    def get_kth_largest(self, k):
        if (k >= len(self)):  # Special case 1
            return self.first()
        if (k < 0):           # Special case 2
            return self.last()
        
        datas = list(self.inorder())  # Use inorder to get smallest to largest. Or make your own traversal!
        datas.reverse()
        return datas[k - 1]
