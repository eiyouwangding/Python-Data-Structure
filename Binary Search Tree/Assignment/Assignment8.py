...
class BinarySearchTree(Tree):
    ...
    def largest_to_smallest(self):
        ...
        result = []
        r = self.root()
        self._largest_to_smallest(r, result)
        return result

    def _largest_to_smallest(self, node, result):
        n = node.element()
        if node._right:
            self._largest_to_smallest(node._right, result)
        result.append(n)
        if node._left:
            self._largest_to_smallest(node._left, result)
    ...
    def LCA(self, node1, node2):
        ...
        a = self._root
        while (a._element - node1._element) * (a._element - node2._element) > 0:
            if node1._element > a._element:
                a = a._right
            else:
                a = a._left
        return a
    '''def _LCA(self, ancestor, node1, node2):
        if node1._element > node2._element:
            node1, node2 = node2, node1
        if ancestor:
            return
        if ancestor._element > node1._element and ancestor._element < node1._element:
            return ancestor
        elif ancestor._element > node1._element and ancestor._element > node1._element:
            return self._LCA(ancestor._left, node1, node2)
        elif ancestor._element < node1._element and ancestor._element < node1._element:
            return self._LCA(ancestor._right, node1, node2)'''
